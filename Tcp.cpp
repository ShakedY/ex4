...
***********************************************************************/
Tcp::Tcp(bool isServers, int port_num,string ip) : Socket(ip) {
...
void Tcp::setDescriptor(int descriptor) {
	this->descriptorCommunicateClient = descriptor;
}
...
Tcp* Tcp::acceptClient() {
	...
	if (descriptor < 0) {
		//return an error represent error at this method
		cout <<"There is an error while accepting client.";
		return NULL;
		}
	Tcp* socket = new Tcp(true,0);
...
int Tcp::sendData(char data) {
	//Do nothing for now.
	//int data_len = data.length();
	//const char * datas = data.c_str();
	int sent_bytes = send(this->isServer ? this->descriptorCommunicateClient
		: this->socketDescriptor, &data, 1, 0);
	if (sent_bytes < 0) {
		//return an error represent error at this method
			return ERROR_SEND;
		}
